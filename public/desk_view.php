...
<section class="panel map-panel">
  ...
  <div id="unassignedZone" style="min-height:72px;border:2px dashed var(--border);border-radius:10px;padding:8px">
    ...
    <div class="drag-order" data-order-id="<?=(int)$o['id']?>" data-from-trip="" data-weight="<?=(float)$o['weight_kg']?>"
      style="display:flex;gap:8px;padding:8px;margin-bottom:6px;border:1px solid var(--border);border-radius:8px;background:var(--order-bg)">
      <div style="flex:1;min-width:0">
        <div style="font-weight:700"><?=h($o['number']?:$o['external_id'])?></div>
        <div class="muted" style="font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?=h($o['address'])?></div>
      </div>
      <span><?=number_format((float)$o['weight_kg'],0,'.',' ')?> кг</span>
    </div>
