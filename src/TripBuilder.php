...
{
    /**
     * Пересобирает draft-рейсы на дату: сначала домашние ТС зоны, до capacity_kg.
     */
    public static function rebuild(PDO $pdo, string $date): array
    {
        $pdo->beginTransaction();
        try {
            // Удаляем только draft на эту дату
            ...
            foreach ($byZone as $zoneId => $zoneOrders) {
                ...
                foreach ($vehicles as $vehicle) {
                    ...
                    if (!$batch) {
                        // Ни одна заявка не влезает целиком — берём первую с перегрузом
                        if ($queue) {
                            $batch[] = $queue[0];
